...
namespace WebDriverBasic.po
{
    class ProductPage
    {
        ...
        public bool GetDiscontinuedValue()
        {
        ...
        {
            FieldProductName.SendKeys(name);
        }

        public void InputCategoryName(string category)
        {
            FieldCategoryId.SendKeys(category);
        }

        public void InputSupplierName(string supplier)
        {
            FieldSupplierId.SendKeys(supplier);
        }

        public void InputUnitPrice(int price)
        {
            FieldPriceUnit.SendKeys(price.ToString());
        }

        public void InputQuantityPerUnit(string quantity)
        {
            FieldQuantityPerUnit.SendKeys(quantity);
        }

        public void InputUnitsInStock(int stock)
        {
            FieldUnitsInStock.SendKeys(stock.ToString());
        }

        public void InputUnitsOnOrder(int order)
        {
            FieldUnitsOnOrder.SendKeys(order.ToString());
        }

        public void InputReorderLevel(int level)
        {
            FieldReorderLevel.SendKeys(level.ToString());
        }

        public void ClickOnDiscontinued(string discontinued)
        {
            if (String.Compare(discontinued, "true") == 0)
                CheckBoxDiscontinued.Click();
        }
    }
}
